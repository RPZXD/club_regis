  ...
  <div class="content-header">
    ...
    <section class="content">
        <div class="container-fluid px-4">
            <!-- Hero Section -->
            <div class="mb-8">
                <div class="bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-700 rounded-2xl p-8 text-white shadow-2xl animate__animated animate__fadeInDown">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-3xl font-bold mb-2">🏆 ตั้งค่าวันเวลาสมัครกิจกรรม Best</h2>
                            <p class="text-emerald-100 text-lg">จัดการช่วงเวลาเปิด-ปิดรับสมัครกิจกรรม Best สำหรับแต่ละระดับชั้น</p>
                        </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 animate__animated animate__fadeInUp">
                <div class="bg-gradient-to-r from-emerald-500 to-teal-600 px-8 py-6">
                    <h3 class="text-2xl font-bold text-white flex items-center">
                        ...
                        การตั้งค่าวันเวลาสมัคร Best
                    </h3>
                </div>
                <div class="p-8">
                    <div class="bg-emerald-50 border-l-4 border-emerald-400 p-4 mb-8 rounded-r-lg">
                        <div class="flex items-center">
                            ...
                            <p class="text-emerald-800 font-medium">กำหนดวันเวลาในการเปิดและปิดรับสมัครกิจกรรม Best สำหรับแต่ละระดับชั้น</p>
                        </div>
                    </div>
                    
                    <form id="bestSettingForm">
                                        ...
                                        <div class="space-y-6">
                                            <div class="relative">
                                                ...
                                                <input type="datetime-local" 
                                                       ...
                                                       id="best_regis_start_<?php echo $level; ?>" 
                                                       name="best_regis_start[<?php echo $level; ?>]" 
                                            ...
                                            <div class="relative">
                                                <label class="block text-sm font-semibold text-gray-700 mb-2 flex items-center">
                                                    <span class="mr-2">🏁</span>
                                                    วันเวลาปิดรับสมัคร Best
                                                </label>
                                                <input type="datetime-local" 
                                                       class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-<?php echo $color[2]; ?>-400 focus:ring-4 focus:ring-<?php echo $color[2]; ?>-100 transition-all duration-300 bg-gray-50 hover:bg-white" 
                                                       id="best_regis_end_<?php echo $level; ?>" 
                                                       name="best_regis_end[<?php echo $level; ?>]" 
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 justify-center mt-12">
                            <button type="submit" class="group bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-bold py-4 px-8 rounded-2xl shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 flex items-center justify-center">
                                <span class="mr-3 group-hover:scale-110 transition-transform duration-300">💾</span>
                                บันทึกการตั้งค่า Best
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 animate__animated animate__fadeInUp" style="animation-delay: 0.2s">
                <div class="bg-gradient-to-r from-teal-500 to-cyan-600 px-8 py-6">
                    <h3 class="text-2xl font-bold text-white flex items-center">
                        <span class="mr-3">📊</span>
                        สถานะการรับสมัคร Best ปัจจุบัน
                    </h3>
                </div>
                <div class="p-8">
                                        ...
                                        <?php if ($regis_start && $regis_end): ?>
                                            <div class="text-sm">
                                                ...
                                                if ($now < $start) {
                                                    ...
                                                    echo '<div class="text-gray-500 text-xs">(จนเปิดรับสมัคร Best)</div>';
                                                } elseif ($now >= $start && $now <= $end) {
                                                    $diff = $now->diff($end);
                                                    echo '<div class="text-green-600 font-medium">' . $diff->days . ' วัน ' . $diff->h . ' ชั่วโมง</div>';
                                                    echo '<div class="text-gray-500 text-xs">(จนปิดรับสมัคร Best)</div>';
                                                } else {
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden animate__animated animate__fadeInUp" style="animation-delay: 0.4s">
                <div class="bg-gradient-to-r from-violet-500 to-purple-600 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <h3 class="text-2xl font-bold text-white flex items-center">
                            <span class="mr-3">📝</span>
                            ประวัติการเปลี่ยนแปลงการตั้งค่า Best
                        </h3>
                        <button type="button" class="bg-white/20 hover:bg-white/30 text-white font-medium py-2 px-4 rounded-xl transition-all duration-300 flex items-center" onclick="loadBestSettingLogs()">
                            <span class="mr-2">🔄</span>
                            รีเฟรช
                        </button>
                    </div>
...
$(document).ready(function() {
    ...
    $('#bestSettingForm').on('submit', function(e) {
        e.preventDefault();
        ...
        $('.form-control').each(function() {
            ...
            if (fieldName && fieldName.includes('best_regis_')) {
                let level = $(this).attr('name').match(/\[(.*?)\]/)[1];
                let fieldType = $(this).attr('name').includes('best_regis_start') ? 'start' : 'end';
                
                if (fieldType === 'end') {
                    let startField = $('input[name="best_regis_start[' + level + ']"]');
                    let endField = $(this);
                    ...
                    if (startField.val() && endField.val()) {
                        ...
                        if (endDate <= startDate) {
                            isValid = false;
                            errorMessages.push('วันปิดรับสมัคร Best ของ ' + level + ' ต้องมากกว่าวันเปิดรับสมัคร');
                        }
                    }
                }
            }
            ...
            success: function(response) {
                if (response.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'บันทึกสำเร็จ',
                        text: 'การตั้งค่า Best ได้รับการบันทึกเรียบร้อยแล้ว',
                        confirmButtonText: 'ตกลง'
                    }).then(() => {
                        location.reload();
                    });
...
function resetForm() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            document.getElementById('bestSettingForm').reset();
        }
    });
}
...
function loadBestSettingLogs() {
    $('#bestLogsContainer').html('<div class="text-center"><i class="fas fa-spinner fa-spin"></i> กำลังโหลด...</div>');
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                displayBestLogs(response.logs);
            } else {
                $('#bestLogsContainer').html('<div class="alert alert-warning">ไม่สามารถโหลดประวัติได้: ' + response.message + '</div>');
            }
        },
        error: function() {
            $('#bestLogsContainer').html('<div class="alert alert-danger">เกิดข้อผิดพลาดในการโหลดประวัติ</div>');
        }
    });
}

function displayBestLogs(logs) {
    if (logs.length === 0) {
        $('#bestLogsContainer').html('<div class="alert alert-info">ยังไม่มีประวัติการเปลี่ยนแปลง</div>');
        return;
    }
    ...
    logs.forEach(function(log, index) {
        ...
        html += '<h6 class="timeline-title">การเปลี่ยนแปลงการตั้งค่า Best</h6>';
