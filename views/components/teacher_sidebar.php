...
<aside id="sidebar" class="fixed top-0 left-0 z-40 w-72 sm:w-64 h-screen transition-transform duration-300 ease-in-out -translate-x-full lg:translate-x-0">
    <div class="h-full overflow-y-auto bg-gradient-to-b from-indigo-900 via-indigo-950 to-black">
        ...
        <div class="px-6 py-8 border-b border-white/5">
            <div class="flex items-center justify-between">
                <a href="index.php" class="flex items-center space-x-4 group flex-1">
                    <div class="relative">
                        <div class="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full blur-lg opacity-40 group-hover:opacity-70 transition-opacity"></div>
                    ...
                    <div>
                        <span class="text-xl font-black text-white tracking-tight">ครูที่ปรึกษา</span>
                        <p class="text-[10px] font-bold text-indigo-400 tracking-[0.2em] uppercase">Teacher Portal</p>
                    </div>
                </a>
        ...
        <div class="px-6 py-4 border-b border-white/5">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-black shadow-lg">
                    <?php echo mb_substr($_SESSION['username'] ?? 'T', 0, 1, 'UTF-8'); ?>
                </div>
                <div>
                    <p class="text-sm font-bold text-white"><?php echo $_SESSION['username'] ?? 'ครู'; ?></p>
                    <p class="text-[10px] text-gray-400">ครูที่ปรึกษาชุมนุม</p>
                </div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <nav class="mt-2 px-3 pb-24">
            <div class="mb-4 px-4">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">เมนูครู</p>
            </div>
                ...
                <li>
                    <a href="<?php echo htmlspecialchars($menu['url']); ?>" 
                       class="sidebar-item flex items-center px-4 py-3 rounded-2xl transition-all group active:scale-[0.98] <?php echo $isActive ? 'bg-white/10 text-white border border-white/5 shadow-xl shadow-black/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'; ?>">
                        ...
                        <?php if($isActive): ?>
                            <div class="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]"></div>
                        <?php endif; ?>
                    </a>
                </li>
                <?php endforeach; ?>
        ...
        <div class="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent">
            <div class="text-center">
                ...
                <p class="text-[8px] text-gray-700 mt-1 font-bold italic opacity-50 uppercase tracking-tighter">Teacher Portal v2.0</p>
            </div>
        </div>
    </div>
</aside>
